...
import blurmap from "../../../assets/images/blurmap.png"; // Imatge de fons per al BlurView
...
export default function MoreReptes() {
  ...
  return (
    <>
                ...
                <XStack key={i} gap="$2">
                  {[0, 1, 2].map((offset) => {
                    ...
                    return (
                      <YStack
                        key={repte?.id ?? index}
                        onPress={() => router.push(`/repte/${repte?.id}`)}
                        items="center"
                      >
      ...
      {original?.es_copiable && (
        <YStack position="absolute" b={0} l={0} r={0} pb={100} items="center">
          <Button
            width="90%"
            onPress={() => {
...
const CopiarRepteDialog = ({
  ...
  const [visibilitat, setVisibilitat] = useState<"PUBLIC" | "AMICS" | "PRIVAT">(
    "PUBLIC"
  );
    ...
    <Dialog modal open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          gap="$4"
        >
          <Dialog.Title fontSize={20}>Vols copiar aquest repte?</Dialog.Title>
